...
| `observation_max_swap_bytes` | int or null | no | Peak swap that the run held, in bytes. Every benchmark type reports it. Contained in `observation_max_host_bytes`, never added to it. Must not be negative (`400` otherwise). A value of `0` is a real reading — the platform sampled swap and the run stayed resident. Null means nothing sampled it |
| `observation_max_host_bytes` | int or null | no | Peak host memory that the run held, in bytes, counting pages the kernel compressed or paged out where the platform can see them. Every benchmark type reports it. Must not be negative (`400` otherwise). Distinct from `max_host_bytes`, the measurement that the peak-memory benchmarks require — the two disagree by design ([why](benchmarks.md#per-benchmark-observation-columns)). Null from a client that does not sample memory |
